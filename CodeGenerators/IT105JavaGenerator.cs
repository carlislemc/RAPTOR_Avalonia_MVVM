...
namespace IT105Java
{
    public class IT105JavaGenerator : raptor.Generate_Interface, CodeGenerators.Imperative_Interface
    {
        ...
        public string FileCheck(string file_to_check)
        {
            ...
            if (File.Exists(file_to_check))
            {
                ...
                Dispatcher.UIThread.InvokeAsync(async () => {
                    ...
                    await MessageBoxClass.Show("File " + file_name + " already exists.  Click OK to overwrite or Cancel.", "Overwrite Warning!", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning);

                    if (mw.buttonAnswer == ButtonResult.Yes)
                    {
                        
                    }
                    else if (mw.buttonAnswer == ButtonResult.No)
                    {
        ...
        {
            string javamethod = Lexer.get_image((Token_Type)name);
        ...
        public void Finish()
        {
            ...
            {*/
            MessageBoxClass.Show("generated: " + file_name);
            //}
        }
        #endregion
